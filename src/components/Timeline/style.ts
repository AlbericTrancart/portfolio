...
import { breakpoints, colorPalette, radius } from 'stylesheet';
...
const Grid = (size: number) => `${size * 5}px`;

export const TimelineContent = styled.main`
  display: flex;
...
export const Item = styled.article`
  ...
  @media screen and (max-width: calc(${breakpoints.lg} - 1px)) {
    ${rightStyle};
    transform: translateX(
      ${Grid(CIRCLE_SIZE / 2 + MARGIN_BETWEEN_ITEMS_AND_TIMELINE)}
    );
    ${ItemContent} {
